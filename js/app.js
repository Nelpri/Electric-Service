...
async function retrocederEstado(id, estadoActual) {
  const idx = ESTADOS.indexOf(estadoActual);
  if (idx <= 0) return;
...
function botonesEstado(item) {
  ...
  const colores = {
    "Pendiente":  { bg: "#fff8e1", border: "#ffe082", text: "#f57f17" },
    "En proceso": { bg: "#e3f2fd", border: "#90caf9", text: "#1565c0" },
    "Finalizado": { bg: "#e8f5e9", border: "#a5d6a7", text: "#2e7d32" },
  };
  const col = colores[item.estado] || colores["Pendiente"];
  const siguiente = siguienteEstado(item.estado);
  const textosAvance = {
    "Pendiente":  "▶ Iniciar trabajo",
    "En proceso": "✔ Marcar finalizado",
  };
  return `
    <div style="display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;margin-top:.6rem">
      <span style="
        font-size:.75rem;font-weight:700;padding:.3rem .75rem;border-radius:20px;
        background:${col.bg};color:${col.text};border:1px solid ${col.border};
      ">${item.estado}</span>
      ${idx > 0 ? `
        <button type="button" class="btn-retroceder-estado" data-id="${item.id}" data-estado="${item.estado}"
          style="background:none;border:1px solid var(--color-border);color:var(--color-text-muted);
          padding:.3rem .6rem;border-radius:20px;font-size:.75rem;cursor:pointer;">
          ↩ Volver atrás
        </button>` : ""}
      ${siguiente ? `
        <button type="button" class="btn-avanzar-estado" data-id="${item.id}" data-estado="${item.estado}"
          style="background:${col.text};color:#fff;border:none;
          padding:.4rem .9rem;border-radius:20px;font-size:.8125rem;font-weight:700;cursor:pointer;
          box-shadow:0 2px 8px rgba(0,0,0,.12);">
          ${textosAvance[item.estado] || `Avanzar a ${siguiente}`}
        </button>` : ""}
    </div>
  `;
}
...
// ==============================
function construirFormEdicion(registro) {
  const wrap = document.createElement("div");
  wrap.className = "edicion-editor";
  wrap.style.cssText = `
  ...
  wrap.innerHTML = `
    ...
    <div style="display:flex;flex-direction:column;gap:.5rem">
      <input type="text" class="ed-nombre" value="${escapeHtml(registro.nombre || "")}" placeholder="Nombre completo"
        style="padding:.55rem .7rem;font-family:inherit;font-size:.875rem;color:var(--color-text);background:var(--color-input-bg);border:1px solid var(--color-border);border-radius:var(--radius-sm)">
      <input type="text" class="ed-telefono" value="${escapeHtml(registro.telefono || "")}" placeholder="Teléfono / WhatsApp"
        style="padding:.55rem .7rem;font-family:inherit;font-size:.875rem;color:var(--color-text);background:var(--color-input-bg);border:1px solid var(--color-border);border-radius:var(--radius-sm)">
      <textarea class="ed-obs" placeholder="Observaciones" rows="3"
        style="padding:.55rem .7rem;font-family:inherit;font-size:.875rem;color:var(--color-text);background:var(--color-input-bg);border:1px solid var(--color-border);border-radius:var(--radius-sm);resize:vertical">${escapeHtml(registro.obs || "")}</textarea>
    </div>
    <div style="display:flex;gap:.5rem;margin-top:.75rem;flex-wrap:wrap">
      <button type="button" class="btn-guardar-edicion" style="
      ...
      <button type="button" class="btn-cancelar-edicion" style="
  ...
  `;
  wrap.querySelector(".btn-guardar-edicion").addEventListener("click", async () => {
    const nombre   = wrap.querySelector(".ed-nombre").value.trim();
    const telefono = wrap.querySelector(".ed-telefono").value.trim();
    const obs      = wrap.querySelector(".ed-obs").value.trim();
    if (!nombre) { alert("El nombre no puede quedar vacío."); return; }
    const btnG = wrap.querySelector(".btn-guardar-edicion");
    btnG.disabled    = true;
    btnG.textContent = "Guardando...";
    try {
      const { error } = await supabaseClient
        .from("registros")
        .update({ nombre, telefono, obs })
        .eq("id", registro.id);
      if (error) throw error;
      wrap.remove();
      await cargarRegistros();
  ...
  wrap.querySelector(".btn-cancelar-edicion").addEventListener("click", () => wrap.remove());
  return wrap;
}
// ==============================
...
async function cargarRegistros() {
  try {
    ...
    data.forEach(item => {
      ...
      li.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:.5rem;flex-wrap:wrap;margin-bottom:.25rem">
          <div>
        ...
        ${botonesEstado(item)}
        <div style="font-size:.8125rem;color:var(--color-text-muted);line-height:1.8;margin-top:.6rem">
        ...
        <div class="acciones-tarjeta" style="display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.75rem;padding-top:.65rem;border-top:1px solid var(--color-border)">
          ...
          <button type="button" class="btn-toggle-edicion"
            data-id="${item.id}"
            data-nombre="${escapeHtml(item.nombre || "")}"
            data-telefono="${escapeHtml(item.telefono || "")}"
            data-obs="${escapeHtml(item.obs || "")}"
            style="display:inline-flex;align-items:center;gap:.35rem;background:rgba(126,176,224,.15);color:var(--color-accent-mid);border:1px solid var(--color-border-focus);padding:.4rem .9rem;border-radius:20px;font-size:.8rem;font-weight:600;cursor:pointer;">
            ✏️ Editar datos
          </button>
          <button
        ...
        <div class="edicion-editor-container"></div>
      `;
      lista.appendChild(li);
    });
    // Avanzar estado
    document.querySelectorAll(".btn-avanzar-estado").forEach(btn => {
      btn.addEventListener("click", function () {
        avanzarEstado(this.dataset.id, this.dataset.estado);
      });
    });
    // Retroceder estado
    document.querySelectorAll(".btn-retroceder-estado").forEach(btn => {
      btn.addEventListener("click", function () {
        retrocederEstado(this.dataset.id, this.dataset.estado);
      });
    });
    // Toggle presupuesto
    document.querySelectorAll(".btn-toggle-pres").forEach(btn => {
      btn.addEventListener("click", function () {
        ...
        // cerrar editor de datos si está abierto en la misma tarjeta
        li.querySelector(".edicion-editor-container").innerHTML = "";
        container.innerHTML = "";
        container.appendChild(construirFormPresupuesto(registroId, items));
        container.scrollIntoView({ behavior: "smooth", block: "nearest" });
      });
    });
    // Toggle edición datos básicos
    document.querySelectorAll(".btn-toggle-edicion").forEach(btn => {
      btn.addEventListener("click", function () {
        const li        = this.closest("li");
        const container = li.querySelector(".edicion-editor-container");
        if (container.querySelector(".edicion-editor")) {
          container.innerHTML = "";
          return;
        }
        // cerrar editor de presupuesto si está abierto en la misma tarjeta
        li.querySelector(".pres-editor-container").innerHTML = "";
        const registro = {
          id: this.dataset.id,
          nombre: this.dataset.nombre,
          telefono: this.dataset.telefono,
          obs: this.dataset.obs,
        };
        container.innerHTML = "";
        container.appendChild(construirFormEdicion(registro));
        container.scrollIntoView({ behavior: "smooth", block: "nearest" });
      });
    });
  } catch (err) {
